...
start() {
	...
	if [ -f $lockfile ];then	
		 ...
		 daemon /usr/local/python2.7/bin/python $base_dir/manage.py runserver 0.0.0.0:80 &>> /tmp/jumpserver.log 2>&1 &
		 daemon /usr/local/python2.7/bin/python $base_dir/run_websocket.py &> /dev/null 2>&1 &
         sleep 4
